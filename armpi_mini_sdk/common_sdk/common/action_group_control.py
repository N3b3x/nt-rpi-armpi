...
class ActionGroupController:
    ...
    def runAction(self, actNum):
        '''
            运行动作组，无法发送stop停止信号(run action group, but the 'stop' signal can not be sent)
            :param actNum: 动作组名字 ， 字符串类型(action name in the type of string)
            :return:
        '''
        if actNum is None:
            return
            
        actNum = "/home/pi/ArmPi_mini/action_groups/" + actNum + ".d6a"
        self.stopRunning = False
