...
class PacketFunction(enum.IntEnum):
    # 可通过串口实现的控制功能(the control function can be achieved via the serial port)
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1  # LED控制(LED control)
    PACKET_FUNC_BUZZER = 2  # 蜂鸣器控制(buzzer control)
    PACKET_FUNC_MOTOR = 3  # 电机控制(motor control)
    PACKET_FUNC_PWM_SERVO = 4  # PWM舵机控制, 板子上从里到外依次为1-4(control PWM servos through the motors numbered 1-4 from inside to outside on the expansion board  )
    PACKET_FUNC_BUS_SERVO = 5  # 总线舵机控制(bus servo control)
    PACKET_FUNC_KEY = 6  # 按键获取(key obtaining)
    PACKET_FUNC_IMU = 7  # IMU获取(IMU obtaining)
    PACKET_FUNC_GAMEPAD = 8  # 手柄获取(gamepad obtaining)
    PACKET_FUNC_SBUS = 9  # 航模遥控获取(model airplane remote control obtaining)
...
class PacketReportKeyEvents(enum.IntEnum):
    # 按键的不同状态(different status of the button)
...
class Board:
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # 子命令为 0x01 设置 SSID, 第二个字节是字符串长度，该长度包含'\0'字符串结束符('0x01 is the sub-command to set SSID, and the second byte represents the length of the string including the terminating '0' character)
...
def pwm_servo_test(board):
    servo_id = 1
    board.pwm_servo_set_position(0.5, [[servo_id, 1500], [3, 1500]])
