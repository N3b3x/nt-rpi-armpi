...
# 6.AI Vision Games Lesson/Lesson 5 Intelligent Stacking
    ...
    while True:
        if __isRunning:
            if detect_color != 'None' and start_pick_up:  # if the block is detected no moving for a period, the robotic arm starts to grip
                ...
                board.pwm_servo_set_position(0.5, [[1, 2000]])# open gripper
                time.sleep(0.6)
                if not __isRunning: # detect if the game is stopped
                    continue
                AK.setPitchRangeMoving((coordinate['capture']), -90,-90, 90, 1000) # robotic arm runs to the gripping position
                time.sleep(1) # delay for 1s
                if not __isRunning:
                    continue
                board.pwm_servo_set_position(0.5, [[1, 1450]])# close the gripper
                time.sleep(0.6)
                if not __isRunning:
                    continue
                ...
                if not __isRunning:
                    continue
                board.pwm_servo_set_position(0.5, [[6, 1500]])# rotate the robotic arm to the position first
                time.sleep(1.5)
                if not __isRunning:
                    continue
                if not __isRunning:
                    continue
                AK.setPitchRangeMoving((coordinate['place'][0], coordinate['place'][1],12), -90,-90, 90, 800) # robotic arm runs to the top of the gripping position
                time.sleep(0.8)
                if not __isRunning:
                    continue
                
                AK.setPitchRangeMoving((coordinate['place'][0], coordinate['place'][1],(coordinate['place'][2]+number*3)), -90,-90, 90, 800) # robotic arm runs to the placing position, the height of the placing position is calculated based on numbers of the stacked blocks
                time.sleep(0.8)
                if not __isRunning: 
                    continue
                board.pwm_servo_set_position(0.5, [[1, 1800]]) # open the gripper to put down the block
                time.sleep(0.6)
                if not __isRunning:
                    continue
                AK.setPitchRangeMoving((6, 0, 18), 0,-90, 90, 1500) # uplift the robotic arm
                time.sleep(1.5)
                if not __isRunning:
                    continue
                board.pwm_servo_set_position(0.5, [[6, 1500]])  # robotic arm returns to the middle
                time.sleep(1.5)
                
                if not __isRunning:
                    continue
                AK.setPitchRangeMoving((0, 8, 10), -90,-90, 90, 800) # robotic arm resets
                time.sleep(0.8)
                 
                number += 1 # accumulate the number of stacking
                if number == 3: # if it equals to 3, reset the counting
                    number = 0
                    board.set_buzzer(1900, 0.1, 0.9, 1)# set the buzzer to sound for 0.1s
                    set_rgb('white')
                    time.sleep(0.5)
