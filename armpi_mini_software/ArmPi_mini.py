...
class MainWindow(QtWidgets.QWidget, Ui_Form):
    ...
    def keyPressEvent(self, event):
        if event.key() == 16777220:
            ...
            data = [[1,servo1_pulse + self.deviation_data[str(1)]],
            ...
            board.pwm_servo_set_position(0.8,data)
            data.clear()
            self.resetServos_ = False
    ...
    def valuechange1(self, name):
        if not self.resetServos_:
            ...
            if name == 'id1':
                ...
                board.pwm_servo_set_position(0.02,[[1,servoAngle1 + self.deviation_data[str(1)]]])
            if name == 'id3':
                servoAngle3 = self.horizontalSlider_3.value()
                self.lineEdit_3.setText(str(servoAngle3))
                board.pwm_servo_set_position(0.02,[[3,servoAngle3 + self.deviation_data[str(3)]]])
            if name == 'id4':
                servoAngle4 = self.horizontalSlider_4.value()
                self.lineEdit_4.setText(str(servoAngle4))
                board.pwm_servo_set_position(0.02,[[4,servoAngle4 + self.deviation_data[str(4)]]])
            if name == 'id5':
                servoAngle5 = self.horizontalSlider_5.value()
                self.lineEdit_5.setText(str(servoAngle5))
                board.pwm_servo_set_position(0.02,[[5,servoAngle5 + self.deviation_data[str(5)]]])
            if name == 'id6':
    ...
    def valuechange2(self, name):
        if not self.resetServos_:
            if name == 'd1':
                ...
                board.pwm_servo_set_position(0.02,[[1,self.horizontalSlider_1.value() + self.devNew[0]]])
            if name == 'd3':
                self.devNew[1] = self.horizontalSlider_13.value()
                self.label_d3.setText(str(self.devNew[1]))
                board.pwm_servo_set_position(0.02,[[3,self.horizontalSlider_3.value() + self.devNew[1]]])
            if name == 'd4':
                self.devNew[2] = self.horizontalSlider_14.value()
                self.label_d4.setText(str(self.devNew[2]))
                board.pwm_servo_set_position(0.02,[[4,self.horizontalSlider_4.value() + self.devNew[2]]])    
            if name == 'd5':
                self.devNew[3] = self.horizontalSlider_15.value()
                self.label_d5.setText(str(self.devNew[3]))
                board.pwm_servo_set_position(0.02,[[5,self.horizontalSlider_5.value() + self.devNew[3]]])
    ...
    def button_re_clicked(self, name):
        ...
        if name == 'reSetServos':
            ...
            data = [[1,1500+self.deviation_data[str(1)]],
            ...
            board.pwm_servo_set_position(1.5,data)
            data.clear()
    ...
    def action_one(self):
        ...
        try:
            ...
            data = [[1,ServoPulse_1+self.deviation_data[str(1)]],
            ...
            board.pwm_servo_set_position(float(timer/1000),data)
            data.clear()
            time.sleep(0.01)
    ...
    def button_flie_operate(self, name):
        try:            
            if name == 'openActionGroup':
                ...
                openfile = dig_o.getOpenFileName(self, 'OpenFile', '/home/pi/ArmPi_mini/action_groups', 'd6a Flies(*.d6a)')
                ...
                # 参数三：默认打开的目录，“.”点表示程序运行目录，/表示当前盘符根目录(parameter 3: default opened directory, '.' represents the program running directory, and '/' represents the root directory of the current disk)
                # 参数四：对话框的文件扩展名过滤器Filter，比如使用 Image files(*.jpg *.gif) 表示只能显示扩展名为.jpg或者.gif文件(parameter 4: File extension filter for the dialog box, for example, using 'Image files (.jpg *.gif)' to display only files with the extension '.jpg' or '.gif')
                # 设置多个文件扩展名过滤，使用双引号隔开；“All Files(*);;PDF Files(*.pdf);;Text Files(*.txt)”(set multiple file extension filters separated by double quotes; for example, 'All Files(*);;PDF Files(*.pdf);;Text Files(*.txt)')
                path = openfile[0]
                try:
                    if path != '':
            ...
            if name == 'tandemActionGroup':
                ...
                openfile = dig_t.getOpenFileName(self, 'OpenFile', '/home/pi/ArmPi_mini/action_groups', 'd6a Flies(*.d6a)')
                # 打开单个文件(open a single file)
                # 参数一：设置父组件；参数二：QFileDialog的标题(parameter 1: set the parent component; parameter 2: the title of 'QFileDialog')
                # 参数三：默认打开的目录，“.”点表示程序运行目录，/表示当前盘符根目录(parameter 3: default opened directory, '.' represents the program running directory, and '/' represents the root directory of the current disk)
                # 参数四：对话框的文件扩展名过滤器Filter，比如使用 Image files(*.jpg *.gif) 表示只能显示扩展名为.jpg或者.gif文件(parameter 4: File extension filter for the dialog box, for example, using 'Image files (.jpg *.gif)' to display only files with the extension '.jpg' or '.gif')
            ...
            if name == 'readDeviation':
                ...
                try:
                    ...
                    data = [[1,ServoPulse1], [3,ServoPulse3], [4,ServoPulse4], [5,ServoPulse5], [6,ServoPulse6]]      
                    board.pwm_servo_set_position(0.15,data)
                    data.clear()
    ...
    # 控制动作组按钮点击事件(the button click event for action group controlling)
    def button_controlaction_clicked(self, name):
        ...
        if name == 'runAction':   # 动作组运行(action group running)
            runAct = threading.Thread(target=AGC.runAction, args=(self.comboBox_action.currentText(), ))
            runAct.start()
    ...
    ################################################################################################
    #获取面积最大的轮廓(obtain the contour with the largest area)
    def getAreaMaxContour(self,contours) :
            ...
            for c in contours :
                contour_area_temp = math.fabs(cv2.contourArea(c)) #计算面积(calculate area)
                if contour_area_temp > contour_area_max : #新面积大于历史最大面积就将新面积设为历史最大面积(If the new area is greater than the historical maximum area, set the new area as the historical maximum area.)
                    contour_area_max = contour_area_temp
                    if contour_area_temp > 10: #只有新的历史最大面积大于10,才是有效的最大面积(The new historical maximum area is only valid if it is greater than 10.)
                                               #就是剔除过小的轮廓(It means to remove the contours that are too small.)
                        area_max_contour = c

            return area_max_contour #返回得到的最大面积，如果没有就是 None(Return the obtained maximum area, or 'None' if there is none.)
            ...
            if ret:
                ...
                mask = cv2.inRange(frame_lab,
                                   ...
                                   (self.current_lab_data[self.color]['max'][0],
                                    ...
                                    self.current_lab_data[self.color]['max'][2]))#对原图像和掩模进行位运算(perform bitwise operation on the orginal image and the mask)
